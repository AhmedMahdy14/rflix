...
def movies(request):
    ...
    if form.is_valid():
        ...
        rated_movie_obj, created = RatingMovie.objects.get_or_create(user=user, movie=movie_obj)
        n_ratings = movie_obj.nratings
        g_rating = movie_obj.rating
        if created:
            movie_obj.rating = (g_rating * n_ratings + p_rating) / (n_ratings + 1)
        ...
        if user_clan:
            ...
            for obj in recommendations:
                if obj.movie not in movies_temp:
                    ...
                    temp = {'movie': obj.movie.title, 'users': {
            ...
            recommendations = Movie.objects.all() \
                                  .exclude(pk__in=[movie.pk for movie in unrecommended_movies]) \
...
def reporting_page(request):
    ...
    # The list of  top 10 users with the number of ratings.
    ...
    sorted_number_of_ratings_for_users = sorted(number_of_ratings_for_users.items(),
    ...
    # The list of popular users the number of party memberships.
    ...
    context = {"n_movies": n_movies,
               ...
               "sorted_number_of_memberships_in_parties_for_users":
                   sorted_number_of_memberships_in_parties_for_users}
...
def login_view(request):
    ...
    if form.is_valid():
        ...
        login(request, user)  # here to be actually logged in.
